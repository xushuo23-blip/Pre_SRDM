...
def srdm_components(rin_reduce="sum", rin_scale=1.0, lambda_cluster=0.0, eps=1e-8):
    """
    Return:
      - r_total (for logging only)
      - metadata dict containing raw components: r_in_raw, r_cluster_raw
    Note:
      Advantages are computed in train.py (component-wise), then linearly mixed.
    """

    def _fn(images, prompts, metadata):
        import numpy as np

        # ---- r_in: from per-sample log_probs stored in metadata ----
        # metadata is a list of dicts, each dict must include "log_probs": shape (T,)
        logp = np.stack([m["log_probs"] for m in metadata], axis=0)  # (N, T)

        if rin_reduce == "sum":
            r_in = logp.sum(axis=1)
        elif rin_reduce == "mean":
            r_in = logp.mean(axis=1)
        else:
            raise ValueError("rin_reduce must be 'sum' or 'mean'")

        r_in = float(rin_scale) * r_in

        # ---- r_cluster placeholder (Stage-5 implement here) ----
        r_cluster = np.zeros_like(r_in)

        # total reward only for logging (NOT used for PPO advantage directly)
        r_total = r_in + float(lambda_cluster) * r_cluster

        info = {
            "r_in_raw": r_in,
            "r_cluster_raw": r_cluster,
            "lambda_cluster": float(lambda_cluster),
        }
        return np.array(r_total), info
