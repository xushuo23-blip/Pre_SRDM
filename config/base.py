...
def get_config():
    ...
    config.prompt_fn = "flux_reasoning_20k"
    # kwargs to pass to the prompt function.
    config.prompt_fn_kwargs = {"path": "data/flux_reasoning_prompts_20k.txt"}

    ###### Reward Function ######
    # reward function to use. see `rewards.py` for available reward functions.
    config.reward_fn = "srdm_components"
    config.reward_fn_kwargs = dict(
    rin_reduce="sum",        # "sum" or "mean"
    rin_scale=1.0,          
    lambda_cluster=0.0,      # >0 means open r_cluster
    eps=1e-8,
)
